...
input_file_dc_path = '../MEx Dataset/Dataset/dc_0.05_0.05'
# Ask the user the patient number
patient_number = input('Enter the patient number (01 - 30): ')
# Ask the user the exercise number
exercise_number = input('Enter the exercise number in the form 0x_dc_x (e.g. 01_dc_1): ')
    ...
    for row in reader:
        ...
        for i in range(11, -1, -1):
            for n in range(0, 16):
                ...
                c = c + 1
        cv2.imwrite(f"frame_{name_count}.jpg", cv2.resize(temp_image * 256, (320, 240)))
